...
import { useIncentiveAudits } from "@/hooks/useIncentiveSchemes";
import type { IncentiveAuditLog } from "@/types/incentive";

const IncentiveAuditTableWrapper = () => {
  const [pagination, setPagination] = useState({ pageIndex: 0, pageSize: 10 });
  const [columnFilters, setColumnFilters] = useState<any[]>([]);
  const [sorting, setSorting] = useState<any[]>([]);
  const [globalFilter, setGlobalFilter] = useState("");
  const [selectedAudit, setSelectedAudit] = useState<IncentiveAuditLog | null>(null);
  ...
  const { data, isLoading, isError, error, refetch, isFetching } = useIncentiveAudits(apiParams);
  ...
  const rowActions = [
    {
      ...
      label: "View Details",
      color: "primary",
      onClick: (row: IncentiveAuditLog) => setSelectedAudit(row),
    },
  ];
  ...
  return (
    <>
      ...
      <AuditDetailDialog
        ...
        title="Scheme Audit Details"
        entityLabel="Scheme Code"
        entityValue={selectedAudit?.scheme_code}
...
export default IncentiveAuditTableWrapper;
