...
export default function SelectAllTransferList() {
  ...
  const customList = (
    ...
    const filteredItems = items.filter((idx) =>
      users[idx]?.toLowerCase().includes(searchValue.toLowerCase())
    );
  ...
  return (
    <div className="mr-auto w-fit rounded-xl border border-gray-200 bg-gradient-to-r from-gray-50 to-white p-6 shadow-sm transition-all duration-300 hover:shadow-md">
      ...
      >
        <Grid>
          {customList("Available Users", left, leftSearch, setLeftSearch)}
        </Grid>
        <Grid>
        ...
        <Grid>
          {customList("Selected Users", right, rightSearch, setRightSearch)}
        </Grid>
      </Grid>
    </div>
  );
