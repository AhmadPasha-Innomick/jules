...
function DynamicTable<T extends object>({
  ...
  useEffect(() => {
    ...
    if (enableServerSide && onColumnFiltersChange) {
      onColumnFiltersChange([]);
    }
  }, [searchField]);

  useEffect(() => {
    if (searchField === "searchby") {
      setSearchValue("");

      if (enableServerSide && onColumnFiltersChange && columnFilters.length > 0) {
        onColumnFiltersChange([]);
        if (refetch) refetch();
      }
    }
  }, [searchField, enableServerSide, onColumnFiltersChange, columnFilters, refetch]);
    ...
    try {
      const filters =
        exportParams && Object.keys(exportParams).length > 0
          ? exportParams
          : {
            ...(searchField !== "searchby" ? { search_by: searchField } : {}),
            ...(searchValue ? { search_value: searchValue } : {}),
          };



      const blob: Blob = await exportUsers(filters as Record<string, any>);
  ...
  const handleSearch = () => {
    ...
    if (!searchField || searchField === "searchby") return;
  ...
  const table = useMaterialReactTable<T>({
    ...
    renderTopToolbarCustomActions: () => (
        ...
        <Box sx={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <TextField
              ...
              onChange={(e) =>
                setSearchField(
                  e.target.value as "searchby" | "user_name" | "email"
                )
              }
              sx={{
                width: 180,
            ...
            <div className="relative">
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && searchField !== "searchby" && searchValue) {
                    handleSearch();
                  }
                }}
                placeholder={
                  searchField === "searchby"
                    ? "Select search type"
                    : `Search by ${searchField === "user_name"
                      ? "Username"
                      : searchField === "email"
                        ? "Email"
                        : ""
                ...
                className="w-64 rounded-md border px-3 py-1.5 pr-8 text-sm outline-none
      disabled:bg-gray-100 disabled:cursor-not-allowed"
            ...
            <Tooltip
              title={
                searchField === "searchby" || !searchValue
                  ? "Select search type & enter value"
                  : "Search"
              }
            >
              <span>
                <IconButton
                  disabled={searchField === "searchby" || !searchValue}
